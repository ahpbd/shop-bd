            ...
            <div class="container">
                <div class="px-2 py-4 px-md-4 py-md-3 bg-white shadow-sm rounded">
                    <div class="mb-3 align-items-baseline" style="text-align: center;">
                        <h3 class="h5 fw-700 mb-0" style="text-transform: uppercase;"><a href="{{ route('products.category', $category->slug) }}">
                            <span class="border-bottom border-primary border-width-2 pb-1 d-inline-block">{{ $category->getTranslation('name') }}</a></span>
                        </h3>
                    ...
                    <div class="aiz-carousel gutters-10 half-outside-arrow" data-items="4" data-xl-items="4" data-lg-items="4"  data-md-items="3" data-sm-items="2" data-xs-items="2" data-arrows='true'>
                        @foreach (get_cached_products($category->id) as $key => $product)
                            <div class="carousel-box">
                                @include('frontend.partials.product_box_1-copy',['product' => $product])
                            </div>
                        @endforeach
                    </div>
                </div>
