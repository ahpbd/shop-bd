...
//        if (Session::has('cart') && count(Session::get('cart')) > 0) {
    ...
    public function store_delivery_info(Request $request)
    {
        ...
        $shipping = 0;
        $subtotal = 0;
     //////// Up 999 Free Delivery condition by Nur/////////
        $subtotal1 = 0;
        $tax1 = 0;

        foreach ($carts as $key => $cartItem) {
        $subtotal1 += $cartItem['price'] * $cartItem['quantity'];
        $tax1 += $cartItem['tax'] * $cartItem['quantity'];

        }
        $sum1 = $subtotal1 + $tax1;
	 //////// Up 999 Free Delivery condition End////////
        ...
        if ($carts && count($carts) > 0) {
            foreach ($carts as $key => $cartItem) {
              ...
              //////////Coupon Condition start///////////////////    
                  //dd($cartItem);
                  //dd($cartItem['coupon_code'] == null);
                  if($cartItem['coupon_code'] == null){
                  ...
                  //dd($cartItem['coupon_applied']);
                }
                //////////Coupon Condition end///////////////////
                
                  //////// Up 999 Free Delivery condition by Nur/////////
                    
                  $campaign = FlashDeal::findOrFail(3);
                         ...
                         $cartItem['shipping_cost'] = getShippingCost($carts, $key);
                          ...
                          if($sum1 > 999) {
                            ...
                            if($cartItem['coupon_applied'] == 1){
                                  $cartItem['shipping_cost'] = getShippingCost($carts, $key);
                            } elseif($cartItem['coupon_applied'] == 0) {
                          $cartItem['shipping_cost'] = 0;
                            }
                          }else{
                          $cartItem['shipping_cost'] = getShippingCost($carts, $key);
                    ...
                    //////// Up 999 Free Delivery condition End by Nur/////////
                }
        ...
        if ($coupon != null) {
            if (strtotime(date('d-m-Y')) >= $coupon->start_date && strtotime(date('d-m-Y')) <= $coupon->end_date) {
                if (CouponUsage::where('user_id', Auth::user()->id)->where('coupon_id', $coupon->id)->first() == null) {
                  ...
                  ////////////coupon apply delivery charge add//////////////
                    foreach ($carts as $key => $cartItem) {
                    }
                  ////////////coupon apply delivery charge add end//////////////
                    //dd($carts);
                    Cart::where('user_id', Auth::user()->id)
                            ->where('owner_id', $coupon->user_id)
                            ->update(
                                    [
                                        ...
                                        'coupon_applied' => 1,
                                        'shipping_cost' => getShippingCost($carts, $key)  ///coupon apply delivery charge add
                                    ]
                    );
